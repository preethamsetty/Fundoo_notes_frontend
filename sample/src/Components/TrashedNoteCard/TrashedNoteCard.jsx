...
const TrashedNoteCard = ({ note, onRestore, onDeleteForever }) => {
  return (
    <div
      className={`trashed-note-card ${note.color ? 'trashed-with-color' : ''}`}
      style={{
        '--note-color': note.color,  // Dynamically apply background color
        '--note-border-color': note.color ? note.color : '#dadce0', // Apply border color if color exists
      }}
    >
      <h3 className="trashed-note-card__title">{note.title}</h3>
      <p className="trashed-note-card__content">{note.description}</p>
